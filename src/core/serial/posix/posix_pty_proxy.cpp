...
bool baudToSpeed(int baud, speed_t &speed) {
    switch (baud) {
        ...
        case 230400:
            speed = B230400;
            return true;
            // B460800 / B921600 are Linux extensions; on platforms that lack the
            // constant (macOS) these rates fall through to the custom-baud path.
#ifdef B460800
        case 460800:
            speed = B460800;
            return true;
...
PosixPtyProxy::PosixPtyProxy(PtyProxy *q) : PtyProxyImpl(q) {}

PosixPtyProxy::~PosixPtyProxy() {
    PosixPtyProxy::close();  // qualified: non-virtual dispatch is intended in a dtor
}

bool PosixPtyProxy::open(const SerialConfig &config) {
    ...
    m_worker = std::thread(&PosixPtyProxy::runLoop, this);

    emit q_ptr->started(m_slavePath);
    return true;
}

bool PosixPtyProxy::configureTermios(const SerialConfig &config) {
    ...
    if (!standardBaud) {
        if (!applyCustomBaud(m_uartFd, config.baud)) {
...
bool PosixPtyProxy::drainLocked(int fd, OutQueue &queue) {
...
bool PosixPtyProxy::forward(int fd, OutQueue &queue, Direction dir, const QByteArray &data) {
    if (data.isEmpty()) {
        return true;
    }
    bool dropped = false;
...
void PosixPtyProxy::mirrorSlaveTermios() {
...
void PosixPtyProxy::wakeLoop() const {
    if (m_wakeWriteFd >= 0) {
        const char byte = 1;
...
void PosixPtyProxy::teardownDescriptors() {
...
bool PosixPtyProxy::injectToDevice(const QByteArray &bytes) {
...
bool PosixPtyProxy::injectToApp(const QByteArray &bytes) {
...
bool PosixPtyProxy::isRunning() const {
    return m_running.load();
}

QString PosixPtyProxy::slavePath() const {
    return m_slavePath;
}

PtyProxy::Stats PosixPtyProxy::stats() const {
    PtyProxy::Stats s;
    s.rx = m_rxBytes.load();
    s.tx = m_txBytes.load();
...
bool PosixPtyProxy::startCapture(const QString &path) {
    QString error;
...
void PosixPtyProxy::stopCapture() {
    m_pcap.close();
}

bool PosixPtyProxy::isCapturing() const {
    return m_pcap.isOpen();
}

PtyProxy::ModemLines PosixPtyProxy::modemLines() const {
...
bool PosixPtyProxy::setRts(bool on) {
    if (m_uartFd < 0) {
        return false;
    }
    int flag = TIOCM_RTS;
...
bool PosixPtyProxy::setDtr(bool on) {
    if (m_uartFd < 0) {
        return false;
    }
    int flag = TIOCM_DTR;
